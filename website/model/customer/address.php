...
class ModelCustomerAddress extends Model {
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			if($address_query->row['city_id']){
				$city_info = $this->db->query("SELECT * FROM `" . DB_PREFIX . "city` WHERE city_id = '" . (int)$address_query->row['city_id'] . "'");
			}
			// print_r($address_query);
			if (!empty($city_info)) {
				$city = $city_info->row['name'];
				$city_code = $city_info->row['code'];
			} else {
				$city = '';
				$city_code = '';
			}
	...
	//
	public function SetPaymentAddress($data,$customer_id)
	{
		$query = $this->db->query("INSERT " . DB_PREFIX . "address SET customer_id = '" . $customer_id . "', name = '" . $data['payment_name'] . "', address = '" . $data['payment_address'] . "', postcode = '" . $data['payment_postcode'] . "', country_id = '" . $data['payment_country_id'] . "', city_id = '" . $data['payment_city_id'] . "', zone_id = '" . $data['payment_zone_id'] . "'");
		return $address_id = $this->db->getLastId();
	}

	public function updateAddress($address_id,$customer_id)
	{
		$update_address = $this->db->query("UPDATE " . DB_PREFIX . "customer SET address_id = '" . $address_id . "' WHERE customer_id = '" . $customer_id . "'");
	}
}

...
class ControllerEventStatistics extends Controller {
	...
	public function addOrderHistory(&$route, &$args, &$output) {
		...
		if ($order_info) {
			...
			if (in_array($args[1], array_merge($this->config->get('shop_processing_status'), $this->config->get('shop_complete_status')))) {
			...
			if (in_array($order_info['order_status_id'], $this->config->get('shop_processing_status')) && !in_array($args[1], $this->config->get('shop_processing_status'))) {
			...
			if (in_array($order_info['order_status_id'], $this->config->get('shop_complete_status')) && !in_array($args[1], $this->config->get('shop_complete_status'))) {
